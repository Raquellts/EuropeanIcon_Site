...
export interface Professor extends PersonBase {
  photo: string;
  curriculum: string;
  areas: string[];
}

export interface Participant extends PersonBase {
  curriculum?: string;
  areas?: string[];
}
...
export const people: Person[] = [...professors, ...participants];
...
export function isProfessor(person: Person): person is Professor {
  return "curriculum" in person && "areas" in person && "photo" in person;
}
