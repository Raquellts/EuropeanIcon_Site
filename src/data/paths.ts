...
/**
 * Foto de uma pessoa: UM único lugar por pessoa, reaproveitado em
 * qualquer contexto (mestrado, evento, revista, etc.) em que ela apareça.
 * Nunca gerar um caminho de foto "por edição" ou "por mestrado" — isso
...
/** Banner específico do coordenador de um mestrado (foto diferente da padrão). */
export function masterBannerPath(masterSlug: string, file: string): string {
  return `/masters/${masterSlug}/${file}`;
}
